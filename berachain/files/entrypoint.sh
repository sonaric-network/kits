#!/bin/sh
  if [ -z "$(find /bin/config/beacond -mindepth 1 -print -quit)" ]; then
    ...
    echo "/bin/config/beacond is not empty, skipping initialization commands."
  fi

  if [ "$DOWNLOAD_SNAPSHOTS" = true ] && [ -n "$(find /snapshots/tmp/beacond -mindepth 1 -print -quit)" ] && [ -n "$(find /snapshots/tmp/reth -mindepth 1 -print -quit)" ]; then
